...
def index():
    ...
    # Chart: Country on x-axis, Export_Value_USD on y-axis, grouped by Year
    if chart_type == "bar":
        fig = px.bar(df, x="Country", y="Export_Value_USD", color="Year", title="Coffee Export Value by Country")
    elif chart_type == "scatter":
        fig = px.scatter(df, x="Country", y="Export_Value_USD", color="Year", title="Coffee Export Value (Scatter)")
    else:
        fig = px.box(df, x="Country", y="Export_Value_USD", color="Year", title="Coffee Export Value (Box Plot)")
